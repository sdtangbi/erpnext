...
def get_columns(filters):
    ...
    columns += [_("Total Present") + ":Float:100",
                _("Total Absent") + ":Float:100"]
    return columns
...
def get_conditions(filters):
    ...
    conditions = " and month(date) = %(month)s and year(date) = %(year)s and cost_center = \'" + \
        str(filters.cost_center) + "\' "
...
def get_years():
    year_list = frappe.db.sql_list(
        """select distinct YEAR(date) from `tabAttendance Others` ORDER BY YEAR(date) DESC""")
    if not year_list:
