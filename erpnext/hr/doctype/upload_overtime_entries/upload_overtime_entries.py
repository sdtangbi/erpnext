...
def get_loaded_records(args, start_date, end_date):
    ...
    for r in rl:
        loaded_list.setdefault(r.employee_type, frappe._dict()).setdefault(
            r.employee, frappe._dict()).setdefault(r.day_of_date, r.number_of_hours)
    ...
    for i, row in enumerate(rows[4:]):
        ...
        try:
            ...
            for j in range(9, len(row) + 1):
                ...
                old = frappe.db.get_value("Overtime Entry", {"number": str(row[4]).strip('\''), "date": str(
                    row[6]) + '-' + str(month) + '-' + str(day), "docstatus": 1}, ["docstatus", "name", "number_of_hours"], as_dict=1)
