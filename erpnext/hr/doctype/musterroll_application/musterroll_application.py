...
class MusterRollApplication(Document):
    ...
    def default_validations(self):
        for i in self.items:
            if not i.joining_date:
                frappe.throw(_("Row#{0} : Date of joining is mandatory.").format(
                    i.idx), title="Missing Value")

            if not i.citizenship_id and not i.existing_cid:
                frappe.throw(_("Row#{0} : Citizenship ID is mandatory.").format(
                    i.idx), title="Missing Value")
    ...
    def validate_submitter(self):
        if self.approver != frappe.session.user:
            frappe.throw(
                "Only the selected supervisor can submit this document")

    def check_status(self):
        for a in self.items:
            if not a.approver_status:
                cid = a.citizenship_id if not a.is_existing else a.existing_cid
                frappe.throw(_("Row#{0} : Approval Status cannot be empty for <b>" + str(
                    a.person_name)+"("+str(cid) + ")</b>").format(a.idx), title="Missing Value")
    ...
    def update_requesting_info(self):
        if self.project:
            ...
            self.cost_center = frappe.db.get_value(
                "Project", self.project, "cost_center")
        elif self.branch:
            self.cost_center = frappe.db.get_value(
                "Branch", self.branch, "cost_center")
        else:
            self.branch = frappe.db.get_value(
                "Branch", {"cost_center": self.cost_center}, "name")
    ...
    def remove_mr(self):
        ...
        for i in self.items:
            ...
            from_date, to_date = frappe.get_value("Employee Internal Work History", {
                                                  "parenttype": "Muster Roll Employee", "parent": cid, "reference_docname": self.name}, ["from_date", "to_date"])
            ...
            if from_date:
                ...
                for ao in frappe.get_all("Attendance Others", fields=["name", "date"], filters={"employee_type": "Muster Roll Employee",
                    ...
                    frappe.throw(_("Row#{0} : Unable to cancel as the employee is having attendance entries for the period from {1} till {2}.").format(
                        i.idx, from_date, to_date), title="Dependencies found")

        frappe.delete_doc("Muster Roll Employee", frappe.db.sql_list(
            "select name from `tabMuster Roll Employee` where reference_docname = '{0}'".format(self.name)))
        frappe.delete_doc("Employee Internal Work History", frappe.db.sql_list(
            "select name from `tabEmployee Internal Work History` where reference_docname = '{0}'".format(self.name)))
        ...
        for a in self.items:
            if a.approver_status == 'Approved':
                ...
                try:
                    ...
                    frappe.throw(_('<span style="color: red;">Muster Roll Application Row#{0}: For Employee <b>{1}({2})</b></span>').format(
                        a.idx, cid, a.person_name), title="Validation Error")
